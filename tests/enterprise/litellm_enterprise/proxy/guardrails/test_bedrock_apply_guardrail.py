...
async def test_bedrock_apply_guardrail_success():
    ...
    with patch.object(
        ...
        result = await guardrail.apply_guardrail(
            text="This is a test message with some content", language="en"
        )
    ...
    with patch.object(
        ...
        with pytest.raises(Exception) as exc_info:
            await guardrail.apply_guardrail(
                text="This is blocked content", language="en"
            )
...
async def test_bedrock_apply_guardrail_with_masking():
    ...
    with patch.object(
        ...
        # Test the apply_guardrail method
        result = await guardrail.apply_guardrail(
            text="This is a test message with sensitive content", language="en"
        )
...
async def test_bedrock_apply_guardrail_api_failure():
    ...
    with patch.object(
        ...
        with pytest.raises(Exception) as exc_info:
            await guardrail.apply_guardrail(
                text="This is a test message", language="en"
            )
...
async def test_bedrock_apply_guardrail_endpoint_integration():
    ...
    with patch(
        "enterprise.litellm_enterprise.proxy.guardrails.endpoints.GUARDRAIL_REGISTRY"
    ) as mock_registry:
        # Mock the make_bedrock_api_request method
        with patch.object(
            guardrail, "make_bedrock_api_request", new_callable=AsyncMock
